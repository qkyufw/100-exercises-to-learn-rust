...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    //     Ok(ticket) => ticket,
    //     Err(TicketNewError::TitleErr { info }) => panic!("{}", info),
    //     Err(_) => easy_ticket(title, "Description not provided".to_string(), status),
    // }
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(ticket) => ticket,
        Err(TicketNewError::TitleErr { info }) => panic!("{}", info),
        Err(TicketNewError::DescriptionErr { info }) => {
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleErr { info: ("Title cannot be empty".to_string()) });
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleErr { info: ("Title cannot be longer than 50 bytes".to_string()) });
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionErr { info: ("Description cannot be empty".to_string()) });
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionErr { info: ("Description cannot be longer than 500 bytes".to_string()) });
        }
